...
class Testimonial extends MX_Controller
{
	...
	$uri_segment = 3;
        ...
        $projects = $this->Testimonial_model->get_last_project($this->modul['limit'], $offset)->result(); // ambil data dari db
        $num_rows = $this->Testimonial_model->count_all_num_rows(); // hitung jumlah baris
	    ...
	    $this->table->set_heading('#','No', 'Name', 'Date', 'Action');
            ...
            foreach ($projects as $project)
            {
                ...
                $this->table->add_row
                (
                    form_checkbox($datax), ++$i, $project->name, tgleng($project->dates),
    ...
    function delete($uid)
    {
        ...
        $img = $this->Testimonial_model->get_project_by_id($uid)->row();
        $img = $img->image;
        if ($img){ $img = "./images/testi/".$img; unlink("$img"); }
        
        $this->Testimonial_model->delete($uid); // memanggil model untuk mendelete data
	...
	$this->form_validation->set_rules('tname', 'Category Name', 'required|callback_valid_name');
        ...
        {
            $config['upload_path'] = './images/testi/';
            ...
            if ( !$this->upload->do_upload("userfile")) // if upload failure
            {
                ...
                $project = array('name' => $this->input->post('tname'), 'dates' => $this->input->post('tdate'), 'desc' => $this->input->post('tlocation'),
                                 'image' => null,'url' => $this->input->post('turl1'));
            }
            else
            {
                $info = $this->upload->data();
                $project = array('name' => $this->input->post('tname'), 'dates' => $this->input->post('tdate'), 'desc' => $this->input->post('tlocation'),
                                 'image' => $info['file_name'],'url' => $this->input->post('turl1'));
            }

            $this->Testimonial_model->add($project);
...
//            echo validation_errors();
    ...
    {
        if ($this->Testimonial_model->valid_name($name) == FALSE)
        {
	...
	if ($this->Testimonial_model->validating_name($name,$id) == FALSE)
        {
	...
	$data['form_action'] = site_url($this->title.'/update_process/'.$uid);
        ...
        $project = $this->Testimonial_model->get_project_by_id($uid)->row();
	...
	$data['default']['name']     = $project->name;
        ...
        $data['default']['image']    = base_url()."images/testi/".$project->image;
...
//	$this->session->set_userdata('cnid', $project->id);
	...
	$this->form_validation->set_rules('tname', 'Name', 'required|callback_validating_name');
        ...
        {   
            ...
            $this->Testimonial_model->update($uid, $project);
