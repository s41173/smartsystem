...
//     ============== ajax ===========================
    ...
    public function getdatatable($search=null,$cust='null',$publish='null')
    {
        if(!$search){ $result = $this->Sales_model->get_last($this->modul['limit'])->result(); }
        else {$result = $this->Sales_model->search($cust,$publish)->result(); }
        ...
        if($cek)
        {
          ...
          for ($i=0; $i<$jumlah; $i++)
          {
             if ($type == 'soft') { $this->Sales_model->delete($cek[$i]); }
                    ...
                    $this->attribute_sales->force_delete_by_sales($cek[$i]);
